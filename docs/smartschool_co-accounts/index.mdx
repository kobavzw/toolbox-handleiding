...
- **Stap 4**: Je kan de inloggegevens nu meteen aan alle betrokken personen bezorgen. Klik daarvoor op **Verzend co-accounts**. Daarbij wordt naar elk mailadres waarvoor een co-account werd aangemaakt een mail gestuurd met de nodige informatie. Die tekst kan je overigens zelf instellen via de module Instellingen => Smartschool => Platform X => Tekst co-accounts mail. In die tekst kan je dynamische variabelen gebruiken die bij het verzenden van de mail worden vervangen door de eigenlijke waarden.
...
:::tip Info
Indien je de co-accounts wil aanmaken via Toolbox, zet je die optie in het administratief pakket (Informat of Wisa) best uit.
:::
